...
def submit_problem(
    ...
    if 'solution_cmd' in config.problems[id]:
        ...
        for file in files:
            if file.startswith(solution_prefix):
                session.submit_file(os.path.join(problem_path, file), ejudge_problem_id)
    ...
    for solution_folder_name in SOLUTION_FOLDER_NAMES:
        ...
        if os.path.exists(folder_path):
            ...
            for file in files:
                session.submit_file(os.path.join(folder_path, file), ejudge_problem_id)
...
def submit_contest(
    ...
    for problem in config.problems:
        if "abstract" not in problem:
            submit_problem(contest_id, problem['id'], only_main_correct)
...
def add_subparsers(subparsers):
    ...
    parser_submit_problem.set_defaults(
        func=lambda options: submit_problem(options.contest_id, options.problem_id, options.only_main)
    )
